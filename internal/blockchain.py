...
from internal.app_config import (
    token_abi,
...
if setting["test_mode"]:
    ...
    polygon_rpc_url, private_key, token_address, chain_id = network_config_parse(
        testnet
    )
else:
...
def level_reward_transfer(to_address: str, amount: int, nonce: int, lock_time: int):
    ...
    private_key_str = private_key
    from_address = web3.eth.account.from_key(private_key_str).address
    return lock_transfer(
        from_address, to_address, amount, private_key_str, nonce, lock_time
    )
